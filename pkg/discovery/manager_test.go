...
func TestManager_Discover(t *testing.T) {
	tests := map[string]func() discoverySimTest{
		"2 discoverers unique groups with delayed collect": func() discoverySimTest {
		...
		"2 discoverers unique groups": func() discoverySimTest {
			const numGroups, numTargets = 2, 2
			d1 := prepareMockDiscoverer("test1", numGroups, numTargets)
			d2 := prepareMockDiscoverer("test2", numGroups, numTargets)
			mgr := prepareManager(d1, d2)
		...
		"2 discoverers same groups": func() discoverySimTest {
			const numGroups, numTargets = 2, 2
			d1 := prepareMockDiscoverer("test1", numGroups, numTargets)
		...
		"2 discoverers empty groups": func() discoverySimTest {
		...
		"2 discoverers nil groups": func() discoverySimTest {
